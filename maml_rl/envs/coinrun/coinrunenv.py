...
from maml_rl.envs.subproc_vec_env import VecEnv
...
def init_args_and_threads(cpu_count=4,
    ...
    int_args = np.array([int(is_high_difficulty), 0, int(1), 0, game_versions['standard'], -1, rand_seed]).astype(np.int32)
...
class CoinRunVecEnv(VecEnv):
    ...
    def __init__(self, game_type, num_envs, lump_n=0, default_zoom=5.0):
        ...
        num_channels = 1 if self.use_black_white else 3
